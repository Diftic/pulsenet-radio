...
- [ ] **Service helper G3: convert helper to a Windows Service.** Currently a standalone console exe. Use `Microsoft.Extensions.Hosting.WindowsServices` to host it as a Service. Auto-start on boot, runs as SYSTEM. Service name: `PulseNetHotkey` (mirrors pipe name). Logging swaps from `Console.WriteLine` to `ILogger` via Event Log or a file sink.
- [ ] **Service helper G4: WiX per-machine MSI + ServiceInstall.** Convert `installer/installer.wxs` from per-user to per-machine (`Package InstallScope="perMachine"`, Program Files install root). Add `<ServiceInstall>` + `<ServiceControl>` to install + start the helper service as part of the MSI transaction. Uninstaller must stop + remove the service before file cleanup.
- [ ] **Service helper G5: install/uninstall test + fallback toast.** Run the produced MSI on a clean Windows 11 VM. Verify service auto-starts, player connects to pipe out of the box, F9 works in-game. Uninstall - verify service is removed cleanly. Add a tray balloon when `HotkeyClient.ConnectionStateChanged` fires `connected=false` for >5 seconds on first launch: "Hotkey-in-game unavailable - run Repair Install".
