...
- [ ] **Phase F: URL expiration recovery.** YouTube audio URLs expire ~6 hours after resolution. Listen for `MediaFailed` on the MediaPlayer, re-resolve via YoutubeExplode, resume from last known position. Needed for long listening sessions before merge to master.
- [ ] **Windows Service helper for hotkey delivery (v2.0, locked in).** SYSTEM-level service installs `WH_KEYBOARD_LL` and IPCs the keystroke to the user-mode player. Mirrors Discord's "Discord System Helper" approach for the identical problem. Required so hotkey-during-game keeps working once testers' Windows catches up to the 26200+ policy change. Implementation scope:
    - New `src/Service/` project for the service binary (or a separate exe with `sc.exe` install/uninstall).
    - IPC channel: named pipe is the obvious choice; service → user-mode player with the keystroke payload.
    - User-mode player listens on the pipe, raises the same `HotkeyPressed` event the current `GlobalHotkeyListener` raises so the rest of the app is untouched.
    - MSI switches from **per-user to per-machine** install (Program Files, one UAC at install). Required because services are per-machine. WiX project change in `installer/installer.wxs`: `Package InstallScope="perMachine"`, install service via `<ServiceInstall>` + `<ServiceControl>`.
    - Uninstaller must stop + remove the service before removing files.
    - Discord-style fallback: if service is missing or stopped, player still launches but logs "hotkey-during-game unavailable" and surfaces a tray balloon nudging the user to repair / reinstall.
