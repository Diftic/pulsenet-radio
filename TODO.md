...
After the CefSharp migration was rolled back (see DEVLOG 2026-05-16 entry, archive branch `archive/cefsharp-migration-attempt`), the active architecture is v1.8.2 codebase plus `CoreWebView2.IsMuted = true` on both browser windows + Option 2 native audio half. F8-in-SC root cause is now understood as **classic UIPI** triggered by CIG bumping the RSI Launcher + Star Citizen installs to admin / High-IL (see DEVLOG 2026-05-17 update). v2.0 commits to an elevated helper via Scheduled Task. Branch `feature/native-audio-option2` (local-only) holds the work. Status:
...
- [x] **F8-in-SC root cause identified (updated 2026-05-17).** Primary cause is classic UIPI: CIG bumped the RSI Launcher and Star Citizen installs to admin / High-IL via a recent update, so Medium-IL PulseNet can no longer `WH_KEYBOARD_LL`-hook input destined for those windows. Cleanly explains all observations (binary worked in morning + failed after reboot = CIG pushed IL bump in between; elevation rescued because IL matched; Discord PTT wave = same UIPI wall across newly-admin games; stable Win11 testers fine = haven't taken the CIG update yet). Original "Windows 11 26200+ user-mode LL hook policy tightening" hypothesis demoted to "still plausible contributor but not load-bearing".
- [x] **Click-blocker fix for dead iframe controls.** With audio now coming from `NativeAudioPlayer` and the iframe muted at the WebView2 layer, the YT volume slider / settings cog / next-video button visible in the top of the iframe don't change anything. Widened `#click-blocker-tl` from 645x60 to full 812x60 (entire top row of the iframe) and added `player.unMute(); player.setVolume(100)` to both `onReady` callbacks in `player.js` (createApiPlayer + loadLiveStream branches). Verified on local Debug build.
...
- [x] **Service helper G5: clean install validated.** User rebooted, ran `scripts/build-msi.ps1`, installed v2.0.0 MSI. Player launches from Program Files, scheduled task `PulseNetHotkey` running, helper process up, F9 toggles overlay in launcher / SC focus / in-game on live server. Clean install half done. Uninstall round-trip still to verify whenever we explicitly tear down (not needed before the pre-release ship).
- [x] **v2.0.0-pre1 prerelease shipped.** Published at https://github.com/Diftic/PulseNet-Player/releases/tag/v2.0.0-pre1 with MSI (396 MB) + portable exe (200 MB). Marked as prerelease so `/releases/latest` still returns v1.8.2 for the in-app UpdateChecker on existing v1.x users. Workflow exclusion `!v*-*` prevents the CI tag-push trigger from flipping the prerelease flag via softprops/action-gh-release. Branch `feature/native-audio-option2` pushed to origin for the first time.
- [ ] **License revision (in working tree, awaiting bundling).** `installer/license.rtf` modified to add "Developed by Mallachi" credit and correct the copyright holder from "PulseNet Player" (product) to "Pulse Broadcasting Network" (org). First wording was committed as `1f5d1e5` ("Developed by Mallachi, for PulseNet Broadcasting") and pushed; the simpler iteration ("Developed by Mallachi" plain) sits dirty in the working tree, intentionally uncommitted until tester feedback brings adjacent code changes worth bundling. Won't reach an installer until the eventual stable v2.0.0 build.
- [ ] **Tester feedback round on v2.0.0-pre1.** Beta cohort runs the MSI install + the helper + the audio half + the click-blocker fix. Watch for: (a) hotkey behavior on testers whose CIG installs have/haven't taken the admin-IL bump yet, (b) Wave Link routing of `PulseNet-Player.exe` audio session, (c) any drift / sync regressions in the native audio half during long real-world listening sessions, (d) MSI install/uninstall ergonomics on testers' machines.
...
- [ ] **Crash-recovery SessionStart hook for Claude Code.** Claude Code's JSONL transcript is already a live mirror (written after every message), but a fresh session doesn't read it. Add a `SessionStart` hook script that detects the most-recently-modified JSONL in this project's `.claude/projects/.../` folder other than the current session, and injects a system reminder telling Claude where it is + when to read it (user mentions "crash", "resume", "where were we"). Lets us recover the live state of a crashed session without losing context. Idea from end of 2026-05-16 session.
