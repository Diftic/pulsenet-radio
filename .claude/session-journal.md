...
## Current State
- **Focus:** Two big findings locked in today. (1) WebView2 + 1-line `IsMuted = true` validates as the audio-source-isolation answer (no CefSharp migration needed). (2) F8-in-SC root cause is NOT user-machine-specific: Windows 11 build 26200+ silently skips user-mode WH_KEYBOARD_LL callbacks when the foreground window is game-IL / protected. Tester machines on older Win11 builds don't have the restriction yet but will when their update lands. Next: scope Option 2 (native audio slaved to YT IFrame API), then scope the Windows Service helper for elevated hotkey delivery.
- **Architecture committed for v2.0:**
  1. **Native audio half (Option 2).** WebView2 muted via `CoreWebView2.IsMuted = true` on OverlayWindow and MiniBannerWindow (1-line each, uncommitted working tree). Native player plays the audio attributed to PulseNet-Player.exe, slaved to YT IFrame API events (play / pause / seek / track-change). AudioBridge refactor needed so OBS Media Source workflow keeps working from the native player's session instead of msedgewebview2.exe.
  2. **Windows Service helper.** SYSTEM-level service installs WH_KEYBOARD_LL and IPCs keystrokes to the user-mode player. Required because Windows 11 26200+ kills user-mode LL hooks for game-focused input. Mirrors Discord's "Discord System Helper" approach for the identical problem.
- **Diagnostic chain (locked in):** F8 failure on Mallachi's machine after Nvidia driver update + reboot. Eliminated: Xbox Game Bar, Game Mode, Discord (wasn't running), all NVIDIA background processes, SteelSeries GG. Breakthrough: Discord's own PTT broke simultaneously, with Discord prompting "Discord System Helper" install. Confirmed by running PulseNet elevated: F9 works in RSI Launcher. From non-elevated: zero LL-hook callbacks for launcher-focus / SC-focus, PyCharm-focus continues to work. Side puzzle: elevated CefSharp earlier today didn't work; Mallachi's hypothesis is Windows applies additional restriction to non-Microsoft-recognized hosting runtimes (WebView2 is signed and recognized; CefSharp.BrowserSubprocess is not). Not chasing further; CefSharp archived.
- **Tester impact:** stable Win11 24H2 / 23H2 testers don't have the restriction yet. Current build works for them today. Service helper protects against the rollout.
- **Carry-over:** Drag-stick bug needs re-verification on v1.8.2 baseline (was present on CefSharp builds). Real playlist IDs for 18 stations still not provided. Wave Link routing tester verification still pending. Console Ctrl+C doesn't shut down the player (Generic Host's `Console.CancelKeyPress` handler not wired in WPF mode) - cheap one-line fix for later.
