...
## Current State
- **Focus:** LocalAudioStreamServer ring-buffer + writer-thread rewrite validated end-to-end. Ready to commit + tag v1.8.2.
- **Blocked:** Awaiting tester verification on Wave Link routing (separate thread). Real playlist IDs for 18 stations still not provided.
- **Open:** `onnxruntime-node not available - local inference disabled` warning from ruflo CLI. Install `onnxruntime-node` if/when local neural/embedding inference becomes desired.
