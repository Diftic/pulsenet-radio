...
## Current State
- **Focus:** Option 2 native audio half is materially complete on `feature/native-audio-option2`. Branch is 10 commits ahead of master (never pushed). End-to-end works: muted WebView2 + native MediaPlayer slaved to YT IFrame API events + single OBS source via Window Capture Capture-Audio-BETA + single PulseNet-Player.exe session in Sonar. AudioBridge / LocalAudioStreamServer stack ripped (1254 lines removed) because the inline native playback made OBS pick up audio directly. Next workstream: Phase F (URL expiration recovery) and the Windows Service helper for hotkey delivery.
- **Architecture (end-of-day state):** WebView2 muted on overlay + banner via `CoreWebView2.IsMuted = true`. `NativeAudioPlayer` (Windows.Media.Playback.MediaPlayer + YoutubeExplode 6.6.0) plays audio inline within PulseNet-Player.exe. `HandlePlayerStateChange` drives state=1/2/0 transitions; `HandlePlayerTimeUpdate` runs drift correction at 500 ms threshold with 3-second settle. Live streams route to `PlayLiveAsync` (HLS) via the `isLive` flag; drift correction skipped for live. TFM is `net9.0-windows10.0.19041.0` for WinRT access.
- **Validated end-to-end:** Solaris Classical (Easter egg, wired to `dQw4w9WgXcQ`) and PulseNet LIVE both play cleanly with audio attributed to PulseNet-Player.exe. Play / pause / seek / station-change all sync between iframe and native player. OBS Window Capture single source covers video + audio.
...
- **Service helper deployment scope (locked in 2026-05-16):** Option 3 chosen over manifest-elevate or scheduled-task tricks. Implies MSI switches from per-user to per-machine. Per-machine adds one UAC at install and lets the WiX `<ServiceInstall>` + `<ServiceControl>` elements run the service install in the same transaction. IPC will be a named pipe; service raises the same `HotkeyPressed` event the current `GlobalHotkeyListener` raises, so the rest of the app is untouched.
